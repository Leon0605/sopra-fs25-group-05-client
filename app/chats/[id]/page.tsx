...
const ChatPage: React.FC = () => {
  ...
  const fetchMessages = async () => {
    try {
      ...
      const fetchedMessages: Message[] = await apiService.get<Message[]>(`/chat/${chatId}/${token}`);
            ...
            {messages.map((message) => {
              ...
              return (
                <li key={message.messageId} className={styles.messageItem}>
                  ...
                  <span style={{ fontSize: "small", fontStyle: "italic", color: "gray" }}>
                    {new Date(message.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  </span>
                  <span className={styles.messageContent}>
                    {message.originalMessage}
                    <br />
                    {message.translatedMessage !== message.originalMessage && (
                      <>
                        <i>{message.translatedMessage}</i> 
                      </>
                    )}
                  </span>
                </li>
